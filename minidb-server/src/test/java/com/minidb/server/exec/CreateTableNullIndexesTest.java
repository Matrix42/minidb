...
/**
 * bug #16:QueryExecutor.handleCreate 传 null 作 indexes。TableSchema compact constructor 已把 null 规范化为
 * List.of();此测试锁定「CREATE TABLE 后 indexes() 返回空表而非 NPE」。
 */
class CreateTableNullIndexesTest {
    ...
    void createTableYieldsEmptyIndexesNotNull() {
        ...
        assertTrue(ts.indexes() != null && ts.indexes().isEmpty(), "新建表 indexes 应为空列表(而非 null)");
        // 空索引表上做 DML 也不受影响
