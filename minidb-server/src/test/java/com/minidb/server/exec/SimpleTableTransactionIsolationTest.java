...
/**
 * SimpleTable(无主键表)事务隔离 + 读自身写入(bug #5):事务 A 的 DELETE/UPDATE 改写
 * 全表时,事务 B 不得看到空表;事务 A 自己仍能看到自身变更。
 */
class SimpleTableTransactionIsolationTest {
    ...
    void simpleTableDeleteInTransactionIsIsolatedAndRollsBack() {
        ...
        assertEquals(java.util.Arrays.toString(new int[]{2, 3}),
                java.util.Arrays.toString(own), "事务 A 应看到自己的 DELETE");
        ...
        assertEquals(java.util.Arrays.toString(new int[]{1, 2, 3}),
                java.util.Arrays.toString(other), "未提交 DELETE 不得让其他事务看到空表");
        ...
        assertEquals(java.util.Arrays.toString(new int[]{1, 2, 3}),
                java.util.Arrays.toString(after), "回滚后 base 不变");
    }
}