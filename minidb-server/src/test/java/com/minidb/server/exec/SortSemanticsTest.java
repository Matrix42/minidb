...
/**
 * 验证 VarChar 排序用 UTF-8 字节比较(ValueComparators)与 String.compareTo 语义一致: UTF-8 编码保序,字节字典序 == code
 * point 序。中文/emoji 覆盖非 ASCII 与补充字符。
 */
class SortSemanticsTest {
    ...
    private String[] sortedNames(String valuesSql) {
        ...
        VectorSchemaRoot root =
                ((QueryResult.Rows) executor.execute("SELECT name FROM t ORDER BY name")).data();
