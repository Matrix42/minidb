...
class MaterializedViewTest {
    ...
    void createAndQueryMaterializedView() {
        ...
        VectorSchemaRoot root =
                ((QueryResult.Rows) executor.execute("SELECT id FROM mv ORDER BY id")).data();
    ...
    void createMaterializedViewWithAggregate() {
        ...
        VectorSchemaRoot root =
                ((QueryResult.Rows) executor.execute("SELECT * FROM mv ORDER BY g")).data();
    ...
    void rejectMultiTableJoinMV() {
        ...
        assertThrows(
                ...
                () ->
                        executor.execute(
                                "CREATE MATERIALIZED VIEW mv AS SELECT t1.id FROM t1 JOIN t2 ON t1.id = t2.id"));
    }
    ...
    void truncateBaseTableClearsDependentMV() {
        ...
        // 验证 MV 有数据
        VectorSchemaRoot root = ((QueryResult.Rows) executor.execute("SELECT * FROM mv")).data();
        assertEquals(2, root.getRowCount());
        root.close();
    ...
    void dmlAutoRefreshMv() {
        ...
        root = ((QueryResult.Rows) executor.execute("SELECT id FROM mv ORDER BY id")).data();
