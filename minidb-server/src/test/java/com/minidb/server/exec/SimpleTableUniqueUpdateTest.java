...
/**
 * SimpleTable(WITH type=simple)路径的 UPDATE 也必须校验主键/UNIQUE 唯一性(bug #8),
 * 与 LSM 路径的 validateUpdateUnique 对齐。
 */
class SimpleTableUniqueUpdateTest {
    ...
    void updateToDuplicatePrimaryKeyRejected() {
        executor.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER) WITH ('type' = 'simple')");
        executor.execute("INSERT INTO t VALUES (1, 10), (2, 20)");
        // id 改成已存在的 2 → 主键冲突必须被拒绝
        assertThrows(Exception.class,
                () -> executor.execute("UPDATE t SET id = 2 WHERE id = 1"));
        // 改成不冲突的值应成功
    ...
    void updateToDuplicateUniqueKeyRejected() {
        executor.execute("CREATE TABLE t (id INTEGER, code INTEGER UNIQUE) WITH ('type' = 'simple')");
        executor.execute("INSERT INTO t VALUES (1, 100), (2, 200)");
        // code 改成已存在的 200 → UNIQUE 冲突必须被拒绝
        assertThrows(Exception.class,
                () -> executor.execute("UPDATE t SET code = 200 WHERE id = 1"));
