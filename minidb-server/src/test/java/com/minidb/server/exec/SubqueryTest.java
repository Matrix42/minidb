...
class SubqueryTest {
    ...
    void uncorrelatedInSubquery() {
        ...
        VectorSchemaRoot root = ((QueryResult.Rows) executor.execute(
                "SELECT id FROM a WHERE a.id IN (SELECT b.aid FROM b) ORDER BY id")).data();
    ...
    void isNotDistinctFromPredicate() {
        ...
        VectorSchemaRoot root = ((QueryResult.Rows) executor.execute(
                "SELECT COUNT(*) AS n FROM c WHERE x IS NOT DISTINCT FROM y")).data();
    ...
    void isDistinctFromPredicate() {
        ...
        VectorSchemaRoot root = ((QueryResult.Rows) executor.execute(
                "SELECT COUNT(*) AS n FROM c WHERE x IS DISTINCT FROM y")).data();
        assertEquals(2, ((BigIntVector) root.getVector("n")).get(0));
        root.close();
    }
}
