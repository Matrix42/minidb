...
/**
 * Container schema mounted under the root as "minidb". Exposes every catalog
 * schema as a sub-schema (so {@code other.t} resolves via {@code minidb.other.t})
 * and also exposes the {@code currentSchema}'s tables directly (so unqualified
 * {@code t} resolves via {@code minidb.t}). The catalog reader path is
 * {@code ["minidb"]}; unqualified resolution relies on the table map below.
 */
