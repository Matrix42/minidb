...
public final class MiniDbLogicalRules {
    ...
    /**
     * HepPlanner 阶段规则(不依赖 RelCollationTraitDef),Calcite 标准 Filter/Project 路径。 注意:不含
     * ProjectRemoveRule / CalcRemoveRule —— 两者按「索引恒等」判 trivial,会删掉改名节点 (SELECT a.id AS aid),丢失 JDBC
     * 可见的列别名。
     */
    public static final List<RelOptRule> HEP =
            List.of(
                    ...
                    new FilterJoinRule.FilterIntoJoinRule(
                            false, RelFactories.LOGICAL_BUILDER, FilterJoinRule.TRUE_PREDICATE),
    ...
    /**
     * VolcanoPlanner 阶段规则:依赖 RelCollationTraitDef。 SortRemoveRule 在输入已满足 collation 时移除冗余 Sort,只在
     * trait 注册(VolcanoPlanner)时触发。
     */
    public static final List<RelOptRule> SORT = List.of(SortRemoveRule.Config.DEFAULT.toRule());
