...
/**
 * Produces a {@link MiniDbSortMergeJoin} for any equi-join. Whether each input is actually
 * pre-sorted is decided by the join's own constructor (via {@code MiniDbJoin.coversKeys} on the
 * logical collations), which feeds the internal-sort cost term in {@code computeSelfCost}.
 */
public final class MiniDbSortMergeJoinRule extends ConverterRule {
    ...
    public RelNode convert(RelNode rel) {
        ...
        return new MiniDbSortMergeJoin(
                join.getCluster(),
                traits,
