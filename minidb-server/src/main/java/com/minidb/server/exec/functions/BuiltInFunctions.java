...
public final class BuiltInFunctions {
    ...
    private static void mathFunctions(FunctionRegistry r) {
        ...
        unaryDoubleOrDecimal(
                r, SqlStdOperatorTable.FLOOR, Math::floor, d -> d.setScale(0, RoundingMode.FLOOR));
        unaryDoubleOrDecimal(
                r, SqlStdOperatorTable.CEIL, Math::ceil, d -> d.setScale(0, RoundingMode.CEILING));
    ...
    private static Function roundFunction() {
        return new Function(
                SqlStdOperatorTable.ROUND.getName(),
                List.of(
                        new Overload(
                                List.of(Float8Vector.class),
                                Float8Vector.class,
                                (args, out) ->
                                        Kernels.fillUnaryDouble(
                                                (Float8Vector) args.get(0),
                                                (Float8Vector) out,
                                                BuiltInFunctions::roundHalfAwayFromZero)),
                        new Overload(
                                List.of(DecimalVector.class),
                                DecimalVector.class,
                                (args, out) ->
                                        Kernels.fillUnaryDecimal(
                                                (DecimalVector) args.get(0),
                                                (DecimalVector) out,
                                                d -> d.setScale(0, RoundingMode.HALF_UP))),
                        new Overload(
                                List.of(Float8Vector.class, IntVector.class),
                                Float8Vector.class,
                                (args, out) -> {
                        ...
                        new Overload(
                                List.of(DecimalVector.class, IntVector.class),
                                DecimalVector.class,
                                (args, out) -> {
                        ...
                        new Overload(
                                ...
                                (args, out) ->
                                        roundIntKernel(
                                                (IntVector) args.get(0),
                                                (IntVector) args.get(1),
                                                (IntVector) out)),
                        new Overload(
                                List.of(BigIntVector.class, IntVector.class),
                                BigIntVector.class,
                                (args, out) ->
                                        roundLongKernel(
                                                (BigIntVector) args.get(0),
                                                (IntVector) args.get(1),
                                                (BigIntVector) out))));
    }
    ...
    private static Function absFunction() {
        return new Function(
                SqlStdOperatorTable.ABS.getName(),
                List.of(
                        new Overload(
                                List.of(SmallIntVector.class),
                                SmallIntVector.class,
                                (args, out) ->
                                        Kernels.fillUnaryShort(
                                                (SmallIntVector) args.get(0),
                                                (SmallIntVector) out,
                                                v -> (short) Math.abs(v))),
                        new Overload(
                                List.of(IntVector.class),
                                IntVector.class,
                                (args, out) ->
                                        Kernels.fillUnaryInt(
                                                (IntVector) args.get(0),
                                                (IntVector) out,
                                                Math::abs)),
                        new Overload(
                                List.of(BigIntVector.class),
                                BigIntVector.class,
                                (args, out) ->
                                        Kernels.fillUnaryLong(
                                                (BigIntVector) args.get(0),
                                                (BigIntVector) out,
                                                Math::abs)),
                        new Overload(
                                List.of(Float4Vector.class),
                                Float4Vector.class,
                                (args, out) ->
                                        Kernels.fillUnaryFloat(
                                                (Float4Vector) args.get(0),
                                                (Float4Vector) out,
                                                Math::abs)),
                        new Overload(
                                List.of(Float8Vector.class),
    ...
    /**
     * 双精度 + 定点两种重载合成一个 {@link Function}(注册表按 SqlOperator 覆盖,不能分开 register)。 double 走 Math 域、DECIMAL
     * 走 BigDecimal 域,输出类型随输入类型。
     */
    private static void unaryDoubleOrDecimal(
            FunctionRegistry r,
            SqlOperator op,
    ...
    /**
     * 字符串函数。UPPER/LOWER 走一元 String 核;CHAR_LENGTH/LENGTH 走 StringToInt 核 (结果 INTEGER);CONCAT 是 `||`
     * 算符、CONCAT_FUNCTION 是 `CONCAT(a,b,...)` 变参函数; SUBSTRING 是 3 参单独核。TRIM 不在此注册:其 3 参形式带 SYMBOL
     * 标志字面量,由 {@code RexInterpreter} 专特殊 handler 处理。
     */
                ...
                new Function(
                        ...
                        List.of(
                                new Overload(
                                        ...
                                        IntVector.class,
                                        (args, out) ->
                                                Kernels.fillStringToInt(
                                                        (VarCharVector) args.get(0),
                                                        (IntVector) out,
                                                        s -> s.codePointCount(0, s.length()))))));
    }

    /** 一元字符串函数:VarCharVector → VarCharVector。 */
    private static void unaryString(
            FunctionRegistry r, SqlOperator op, ScalarKernels.StringUnary fn) {
        r.register(
                op,
                new Function(
                        op.getName(),
    ...
    private static void binaryString(
            FunctionRegistry r, SqlOperator op, ScalarKernels.StringBinary fn) {
        r.register(
                op,
                new Function(
                        op.getName(),
                        List.of(
                                new Overload(
                                        ...
                                        VarCharVector.class,
                                        (args, out) ->
                                                Kernels.fillBinaryString(
                                                        (VarCharVector) args.get(0),
                                                        (VarCharVector) args.get(1),
                                                        (VarCharVector) out,
                                                        fn)))));
    }

    /** CONCAT(arg, ...) 变参函数(1+ 参):任一参 null → 结果 null(STRICT)。注册 1/2/3 参 三个常见元数,共用同一个变参核。 */
    private static Function concatFunction() {
        Kernel kernel = BuiltInFunctions::concat;
        return new Function(
                SqlLibraryOperators.CONCAT_FUNCTION.getName(),
                List.of(
    ...
    /**
     * SUBSTRING(s, from, len) 的 3 参核。第二/三参是整型,但可能是 INTEGER(IntVector)或 BIGINT(BigIntVector)列/字面量 ——
     * 两种类型都要能接,故各组合都注册到同一个核,核内用 {@link #longArg} 类型无关地读取出 long 值(再在 slice 里 clamp)。结果恒
     * VARCHAR(VarCharVector)。
     */
    private static Function substringFunction() {
        Kernel substringKernel = BuiltInFunctions::substring;
        return new Function(
                SqlStdOperatorTable.SUBSTRING.getName(),
                List.of(
                        new Overload(
                                ...
                                substringKernel),
                        new Overload(
                                List.of(VarCharVector.class, IntVector.class, BigIntVector.class),
                                VarCharVector.class,
                                substringKernel),
                        new Overload(
                                List.of(VarCharVector.class, BigIntVector.class, IntVector.class),
                                VarCharVector.class,
                                substringKernel),
                        new Overload(
                                List.of(
    ...
    private static long longArg(ValueVector v, int i) {
        ...
        throw new IllegalArgumentException(
                "SUBSTRING operand is not an integer vector: " + v.getClass());
    }

    /**
     * 从 0-based start(字符位,code point)截取最多 length 个字符。start/length 用 long 传入、 在本方法内 clamp 到 [0,
     * codePointCount],避免超大字面量在 int 截断时回绕;负 start 裁剪到 0, 越界或 length<=0 返回空串。按 Unicode code point
     * 截取(非 UTF-16 code unit)。
     */
    ...
    private static Function positionFunction() {
        return new Function(
                SqlStdOperatorTable.POSITION.getName(),
                List.of(
                        new Overload(
                                List.of(VarCharVector.class, VarCharVector.class),
                                IntVector.class,
    ...
    private static Function replaceFunction() {
        return new Function(
                SqlStdOperatorTable.REPLACE.getName(),
                List.of(
                        new Overload(
                                List.of(
                                        VarCharVector.class,
        ...
        return new Function(
                ...
                List.of(
                        new Overload(
                                ...
                                kernel),
                        new Overload(
                                List.of(VarCharVector.class, BigIntVector.class),
                                VarCharVector.class,
                                kernel)));
    }
    ...
    private static void leftRight(List<ValueVector> args, FieldVector out, boolean left) {
        ...
        for (int i = 0; i < str.getValueCount(); i++) {
            ...
            result.setSafe(
                    i,
                    (left ? new String(cp, 0, count) : new String(cp, cp.length - count, count))
                            .getBytes(StandardCharsets.UTF_8));
        }
    }
    ...
    private static Function repeatFunction() {
        ...
        return new Function(
                SqlLibraryOperators.REPEAT.getName(),
                List.of(
                        new Overload(
                                List.of(VarCharVector.class, IntVector.class),
                                VarCharVector.class,
    ...
    private static void repeat(List<ValueVector> args, FieldVector out) {
        ...
        for (int i = 0; i < str.getValueCount(); i++) {
            ...
            result.setSafe(
                    i,
                    (times <= 0 ? "" : s.repeat((int) Math.min(times, Integer.MAX_VALUE)))
                            .getBytes(StandardCharsets.UTF_8));
        }
    }
    ...
    private static String initcap(String s) {
        ...
        for (int code : cp) {
            if (Character.isLetterOrDigit(code)) {
                sb.appendCodePoint(
                        capitalize ? Character.toUpperCase(code) : Character.toLowerCase(code));
                capitalize = false;
            } else {
        ...
        return new Function(
                ...
                List.of(
                        ...
                        new Overload(
                                ...
                                kernel),
                        new Overload(
                                List.of(VarCharVector.class, IntVector.class, VarCharVector.class),
                                VarCharVector.class,
                                kernel),
                        new Overload(
                                List.of(
    ...
    private static String pad(String s, long target, String pad, boolean left) {
        ...
        return left
                ? block.append(new String(cp, 0, cp.length)).toString()
    ...
    private static Function asciiFunction() {
        return new Function(
                SqlStdOperatorTable.ASCII.getName(),
                List.of(
                        new Overload(
                                List.of(VarCharVector.class),
                                IntVector.class,
    ...
    /**
     * CHR(n):code point 转字符。0 返回 NUL 字符(与 Calcite 常量折叠的 charFromUtf8 一致, 保证常量与列求值结果相同;VARCHAR 能存
     * NUL,无需 PostgreSQL 那套「存储受限才返 null」)。
     */
    private static Function chrFunction() {
        Kernel kernel = BuiltInFunctions::chr;
        return new Function(
                SqlLibraryOperators.CHR.getName(),
                List.of(
    ...
    private static Function splitPartFunction() {
        ...
        return new Function(
                SqlLibraryOperators.SPLIT_PART.getName(),
                List.of(
                        new Overload(
    ...
    /**
     * LIKE / NOT LIKE:% 匹配任意序列、_ 匹配单个字符,其余字面量按正则转义后原样。 模式经 likeToRegex 转正则后整体匹配(非部分匹配)。NOT LIKE
     * 是独立算子(SqlLikeOperator negated=true),故分别注册;ESCAPE 子句(3 参形式)暂不支持。
     */
    ...
    private static Function currentDateFunction() {
        ...
        return new Function(
                "CURRENT_DATE", List.of(new Overload(List.of(), DateDayVector.class, kernel)));
    }
    ...
    private static void comparison(FunctionRegistry r) {
        ...
        r.register(
                ...
                comparisonFunction(
                        SqlStdOperatorTable.LESS_THAN_OR_EQUAL, SqlKind.LESS_THAN_OR_EQUAL));
        r.register(
        ...
        r.register(
                ...
                comparisonFunction(
                        SqlStdOperatorTable.GREATER_THAN_OR_EQUAL, SqlKind.GREATER_THAN_OR_EQUAL));
    }

    /**
     * 单个比较运算符的所有重载合成一个 {@link Function}(注册表按 SqlOperator 覆盖)。结果恒 BOOLEAN(BitVector);同型
     * Int/Long/Double 各走对应核,字符串走 String 核。真混型 INTEGER/BIGINT(如 `int_col > bigint_col`)Calcite 不强制
     * CAST,需 [Int,BigInt]/[BigInt,Int] 跨型重载(int 侧 promote 到 long)。
     */
    private static Function comparisonFunction(SqlOperator op, SqlKind kind) {
        return new Function(
                ...
                List.of(
                        new Overload(
                                List.of(SmallIntVector.class, SmallIntVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareShort(
                                                (SmallIntVector) args.get(0),
                                                (SmallIntVector) args.get(1),
                                                (BitVector) out,
                                                Short::compare,
                                                kind)),
                        new Overload(
                        ...
                        new Overload(
                                ...
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareLong(
                                                (BigIntVector) args.get(0),
                                                (BigIntVector) args.get(1),
                                                (BitVector) out,
                                                Long::compare,
                                                kind)),
                        new Overload(
                                List.of(Float4Vector.class, Float4Vector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareFloat(
                                                (Float4Vector) args.get(0),
                                                (Float4Vector) args.get(1),
                                                (BitVector) out,
                                                Float::compare,
                                                kind)),
                        new Overload(
                                ...
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareDouble(
                                                (Float8Vector) args.get(0),
                                                (Float8Vector) args.get(1),
                                                (BitVector) out,
                                                Double::compare,
                                                kind)),
                        new Overload(
                                List.of(DecimalVector.class, DecimalVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareDecimal(
                                                (DecimalVector) args.get(0),
                                                (DecimalVector) args.get(1),
                                                (BitVector) out,
                        ...
                        new Overload(
                                ...
                                (args, out) ->
                                        Kernels.fillCompareString(
                                                ...
                                                kind)),
                        new Overload(
                                List.of(TimeMilliVector.class, TimeMilliVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareTime(
                                                (TimeMilliVector) args.get(0),
                                                (TimeMilliVector) args.get(1),
                                                (BitVector) out,
                                                Integer::compare,
                                                kind)),
                        new Overload(
                                List.of(DateDayVector.class, DateDayVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareDate(
                                                (DateDayVector) args.get(0),
                                                (DateDayVector) args.get(1),
                                                (BitVector) out,
                                                Integer::compare,
                                                kind)),
                        new Overload(
                        ...
                        new Overload(
                                ...
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareBytes(
                                                (VarBinaryVector) args.get(0),
                                                (VarBinaryVector) args.get(1),
                                                (BitVector) out,
                                                Arrays::compareUnsigned,
                                                kind)),
                        new Overload(
                                List.of(IntVector.class, BigIntVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareIntLong(
                                                (IntVector) args.get(0),
                                                (BigIntVector) args.get(1),
                                                (BitVector) out,
                                                Long::compare,
                        ...
                        new Overload(
                                List.of(Float8Vector.class, SmallIntVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                                                args.get(1),
                                                (BitVector) out,
                                                Double::compare,
                                                kind)),
                        new Overload(
                                List.of(SmallIntVector.class, Float8Vector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                        ...
                        new Overload(
                                ...
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                ...
                                                kind)),
                        new Overload(
                                List.of(IntVector.class, Float8Vector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                                                ...
                                                kind)),
                        new Overload(
                                List.of(Float8Vector.class, BigIntVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                                                ...
                                                kind)),
                        new Overload(
                                List.of(BigIntVector.class, Float8Vector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                                                ...
                                                kind)),
                        new Overload(
                                List.of(Float8Vector.class, DecimalVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                                                ...
                                                kind)),
                        new Overload(
                                List.of(DecimalVector.class, Float8Vector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareMixed(
                                                args.get(0),
                        ...
                        new Overload(
                                List.of(BigIntVector.class, VarCharVector.class),
                                BitVector.class,
                                (args, out) ->
                                        Kernels.fillCompareLongString(
                                                (BigIntVector) args.get(0),
                                                (VarCharVector) args.get(1),
                                                (BitVector) out,
                                                Long::compare,
    ...
    /**
     * 单个算术运算符的所有重载合成一个 {@link Function}。每个 SqlOperator 在注册表里只对应 一个 Function({@code register}
     * 会覆盖)。真混型 INTEGER/BIGINT(如 `int_col + bigint_col`) Calcite **不**强制 CAST,操作数保持 Int+BigInt 混型、结果
     * BIGINT,故需跨型重载(输出 BigIntVector,int 侧 promote 到 long)。整数字面量经 RexInterpreter 已是 IntVector(不再 产
     * BigIntVector),故不再有「字面量坑」的 Int 结果跨型。
     */
    private static Function arithmeticFunction(SqlOperator op) {
        ...
        Kernel doubleMixed =
                (args, out) ->
                        fillDoubleMixed(args.get(0), args.get(1), (Float8Vector) out, doubleOp);
        Kernel decimalMixed =
                (args, out) ->
                        fillDecimalMixed(args.get(0), args.get(1), (DecimalVector) out, decimalOp);
        return new Function(
                op.getName(),
                List.of(
                        new Overload(
                        ...
                        // FLOAT × DOUBLE → DOUBLE
                        new Overload(
                                List.of(Float4Vector.class, Float8Vector.class),
                                Float8Vector.class,
                                doubleMixed),
                        new Overload(
                                List.of(Float8Vector.class, Float4Vector.class),
                                Float8Vector.class,
                                doubleMixed),
                        ...
                        // 整型 × DECIMAL → DECIMAL
                        new Overload(
                                List.of(SmallIntVector.class, DecimalVector.class),
                                DecimalVector.class,
                                decimalMixed),
                        new Overload(
                                List.of(DecimalVector.class, SmallIntVector.class),
                                DecimalVector.class,
                                decimalMixed),
                        new Overload(
                                List.of(IntVector.class, DecimalVector.class),
                                DecimalVector.class,
                                decimalMixed),
                        new Overload(
                                List.of(DecimalVector.class, IntVector.class),
                                ...
                                decimalMixed),
                        new Overload(
                                List.of(BigIntVector.class, DecimalVector.class),
                                DecimalVector.class,
                                decimalMixed),
                        new Overload(
                                List.of(DecimalVector.class, BigIntVector.class),
                                DecimalVector.class,
                                decimalMixed)));
    }

    /** 跨族混合算术:两侧都读成 double,结果写 Float8Vector(结果类型为 DOUBLE)。 */
    private static void fillDoubleMixed(
            ValueVector left, ValueVector right, Float8Vector out, ScalarKernels.DoubleBinary op) {
    ...
    private static void fillDecimalMixed(
            ValueVector left,
            ValueVector right,
            DecimalVector out,
        ...
        for (int i = 0; i < left.getValueCount(); i++) {
            ...
            out.setSafe(
                    i,
                    Kernels.scaleTo(out, op.apply(toBigDecimal(left, i), toBigDecimal(right, i))));
        }
    }
    ...
    private static void fillIntLongBinary(
            IntVector left, BigIntVector right, BigIntVector out, ScalarKernels.LongBinary op) {
        for (int i = 0; i < left.getValueCount(); i++) {
            if (left.isNull(i) || right.isNull(i)) {
                out.setNull(i);
                continue;
    ...
    private static void fillLongIntBinary(
            BigIntVector left, IntVector right, BigIntVector out, ScalarKernels.LongBinary op) {
        for (int i = 0; i < left.getValueCount(); i++) {
            if (left.isNull(i) || right.isNull(i)) {
                out.setNull(i);
                continue;
