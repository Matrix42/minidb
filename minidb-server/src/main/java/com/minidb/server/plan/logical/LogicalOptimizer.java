...
public final class LogicalOptimizer {
    ...
    /**
     * 移除 EXISTS/IN/标量子查询:先 SubQueryRemoveRule 把嵌在 Filter/Project 条件里的 RexSubQuery 提升为
     * LogicalCorrelate,再 RelDecorrelator 把 Correlate 去相关为普通 join/聚合(EXISTS → INNER join + 聚合去重;NOT
     * EXISTS → LEFT join + IS NULL 过滤)。 无子查询的查询两阶段都原样返回,不受影响。
     */
    private static RelNode decorrelateSubqueries(RelNode logical) {
        HepPlanner subQueryPlanner =
                new HepPlanner(
        ...
        return RelDecorrelator.decorrelateQuery(
                correlated, RelFactories.LOGICAL_BUILDER.create(correlated.getCluster(), null));
    }
}
