...
public interface TableHandle extends AutoCloseable {
    ...
    /**
     * 主键范围扫描:只返回主键在闭区间 [rangeLo, rangeHi] 内的行(元素 null = 该列无界)。 默认回退全量扫描;LSM 表覆写以利用文件 min/max 与块索引
     * startKey 裁剪。
     */
    ...
    /**
     * 快照读:只返回 snapshotTxId 之前已提交的行。 snapshotTxId == -1 表示 READ_UNCOMMITTED(不过滤)。 默认回退全量扫描;LSMTable
     * 覆写以支持快照隔离。
     */
    ...
    /**
     * 事务快照读 + 自己的写入可见:txId 标识当前事务,其 tx-private 写入(含未提交) 必须对自身可见。默认回退 scan(snapshotTxId);LSMTable
     * 覆写合并自有私有表。
     */
    ...
    /** 提交事务:将 tx-private 写入合并到主存储。 默认空操作;事务感知表覆写。 */
    default void commitTx(long txId) {}

    /** 回滚事务:丢弃 tx-private 写入。 默认空操作;事务感知表覆写。 */
    ...
    /** 按主键点查(仅 LSM 表支持,走 memtable → Bloom → 单 SSTable)。 返回该键的行值;键不存在、已被删除或表不支持点查时返回 null。 */
