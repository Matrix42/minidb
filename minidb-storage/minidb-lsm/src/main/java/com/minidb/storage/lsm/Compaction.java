...
/**
 * Leveled compaction: 合并 L(n) 的 SSTable 到 L(n+1)，按 key 排序并去重
 * （同 key 保留最新版本）。
 */
public class Compaction {
    ...
    public void compactLevel0To1(SSTableManager mgr, TableSchema schema,
        ...
        for (SSTable sst : l0Files) {
            if (overallMin == null || SSTable.KEY_COMPARATOR.compare(sst.minKey(), overallMin) < 0) {
                overallMin = sst.minKey();
            }
            if (overallMax == null || SSTable.KEY_COMPARATOR.compare(sst.maxKey(), overallMax) > 0) {
     ...
     * inputs 的顺序决定同 key 去重时的优先级：前面的文件更新，保留第一个遇到的值。
     */
    private List<SSTable> mergeAndWrite(List<SSTable> inputs, int targetLevel,
        ...
        for (var entry : allRows) {
            if (lastKey == null
                    || SSTable.KEY_COMPARATOR.compare(entry.getKey(), lastKey) != 0) {
