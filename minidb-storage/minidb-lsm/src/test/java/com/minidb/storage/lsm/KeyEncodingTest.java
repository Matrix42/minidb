...
/**
 * 二进制保序 key 编码(A2)回归:整数定长 + 符号位翻转使字节字典序 = 数值序
 * (负数 < 零 < 正数,含 MIN/MAX),字符串长度前缀保序,复合 key 无分隔符;
 * decode 按 schema 主键列类型还原;WAL 恢复类型保真(数字字符串 key 不被误 parse)。
 */
class KeyEncodingTest {
    ...
    void integerKeysOrderByBytes() {
        ...
        assertTrue(byteLess(encodeKey(List.of(Integer.MIN_VALUE)), encodeKey(List.of(Integer.MAX_VALUE))));
        // BIGINT(含 MIN/MAX 边界)
        ...
        assertTrue(byteLess(encodeKey(List.of(Long.MIN_VALUE)), encodeKey(List.of(Long.MAX_VALUE))));
        // SMALLINT
    ...
    void decodeRoundTripsBySchema() {
        TableSchema multi = new TableSchema("public", "mt",
                List.of(new ColumnMeta("a", ColumnType.INTEGER),
        ...
        assertEquals(List.of(Integer.MIN_VALUE, Long.MAX_VALUE, "q"),
    ...
    void walStringKeyNotParsedToInteger(@TempDir Path dir) throws Exception {
        ...
        TableSchema strSchema = new TableSchema("public", "t",
                List.of(new ColumnMeta("id", ColumnType.VARCHAR), new ColumnMeta("v", ColumnType.VARCHAR)),
                List.of("id"), List.of(), List.of());
    ...
    /** 无符号字节比较——encodeKey 翻转符号位后,无符号字典序 = 数值序
     *  (0x80+ 是正数域,Java 的 Arrays.compare 按有符号 byte 比较会反序)。 */
