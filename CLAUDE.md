...
- `exec/RexInterpreter` — RexNode 表达式求值**薄壳**:只处理 RexInputRef(拷列)/RexLiteral(字面量向量)求值 + AND/OR/NOT/CAST/CASE 专用 handler;算术/比较/字符串/数学函数一律经 `functions.lookup(call.getOperator())` 走 `exec/functions` 框架(见坑 47),供 Filter/Project 用。双参构造 `(allocator, FunctionRegistry)` 注入自定义注册表(UDF 扩展点),单参构造默认 `BuiltInFunctions.newRegistry()`。
...
47. **列式标量函数框架(`exec/functions` 包)**:算术(`+ - * /`)/比较(`= != < <= > >=`)/字符串(`UPPER`/`LOWER`/`TRIM`/`CHAR_LENGTH`/`||`/`SUBSTRING`)/数学(`ABS`/`FLOOR`/`CEIL`)不再内联在 `RexInterpreter`,全部走注册表。组件:`Kernel`(`void execute(List<ValueVector>, FieldVector)`,Tier-2 完整核)/`ScalarKernels`(Tier-1 无装箱 lambda 接口 `Int/Long/Double/StringUnary`、`...Binary`、`...Compare`)/`Kernels.fill*`(共享 STRICT-null 循环,任一入参行为 null 就 `setNull` 跳过,只写值**不 setValueCount**)/`Overload`(声明 `inputTypes` + kernel)/`Function`(若干 overload,按输入类型分发)/`FunctionRegistry`(`Map<SqlOperator,Function>`,可变,UDF 挂载点)/`BuiltInFunctions`(把所有内置函数挂到注册表)。`RexInterpreter.evalCall` default 分支:先 `eval` 各操作数成向量,再 `functions.lookup(call.getOperator())` 取 `Function`(找不到抛 UnsupportedOperationException),`f.evaluate(args, call.getType(), allocator)`。**加新函数 = 在 `BuiltInFunctions` 注册一个 `Function`**(typed lambda + `Kernels.fill*` 或 Tier-2 `Kernel`),其余全自动。`Function.evaluate` 负责:逐 overload 用 `inputTypes.get(i).isInstance(arg)` 匹配分发、`ArrowTypes.field(resultType,"expr").createVector(allocator)` 分配输出 + `setInitialCapacity(rows)+allocateNew`、try 内 `kernel.execute`、finally 关闭所有入参向量、**最后 `out.setValueCount(rows)`**(valueCount 由 evaluate 设,`Kernels.fill*` 不设)。两个坑:① **register-overwrite**——`FunctionRegistry.register` 用 `Map.put`,一个 `SqlOperator` 只对应**一个** `Function`,所有重载(同型+跨型)必须收进那一个 `Function` 的 `overloads` 列表,逐个 register 会互相覆盖只剩最后一个(算术/比较/ABS 都用一个合成方法把重载打包成一个 Function);② **整数字面量坑(#23 变体)**——`RexInterpreter.literalVector` 对 INTEGER 字面量恒产 `BigIntVector`、INTEGER 列产 `IntVector`,`id + 1`/`id > 5` 是 Int+BigInt 混型,无同型重载可匹配,必须额外注册 `[Int,BigInt]`/`[BigInt,Int]` 跨型重载(输出硬编码 IntVector 安全:真混型 INTEGER/BIGINT 操作数 Calcite 会在算子前 CAST 成 BIGINT,跨型只由「字面量是 BigIntVector 但 Calcite 类型仍 INTEGER」触发,结果恒 INTEGER)。**已知缺口**:`TRIM` 端到端不通(Calcite 解析期把 `TRIM(s)` 重写成 3 参 `TRIM(Flag,' ',s)`,只注册了 1 参 `String::trim` 核);`ROUND` 未注册(DECIMAL→double 返回类型含糊);`CHAR_LENGTH` 对非 BMP 字符按 UTF-16 code unit 计(`String.length()`)非 code point。另注:`ABS` 的 `[BigIntVector]` 重载核内按 `out` 实际类型分支(INTEGER 字面量 → IntVector 写 `(int) Math.abs`、BIGINT 列 → BigIntVector 写 `Math.abs`),因整数字面量坑使同一输入类型映射到两种输出类型、仅按输入类型分发无法区分(见 `BuiltInFunctions.absBigInt`)。**SQL 关键字映射坑**:`LENGTH(name)` 与 `CONCAT(a,b)` 函数语法**都不被识别**——它们分别在 `SqlLibraryOperators` 的 MYSQL/BIG_QUERY 库(`LENGTH`=`CHAR_LENGTH.withName("LENGTH")`、`CONCAT_FUNCTION` 是变参版),而 `CalciteContext` 只开 `Lex.MYSQL`(纯词法,不挂 MYSQL 函数库),validator 用 `SqlStdOperatorTable.instance()`。端到端只能用标准名:`CHAR_LENGTH(name)`(注意 `CHARACTER_LENGTH`/`LENGTH` 是 `CHAR_LENGTH.withName(...)` 产出的**不同** `SqlOperator` 实例,`SqlOperator.equals` 比 name+kind,`BuiltInFunctions` 只注册了 `CHAR_LENGTH` 这一个 key——`CHARACTER_LENGTH` 能过校验但执行时 `lookup` 返回 null 抛 UnsupportedOperationException,各自需单独注册)、`a || b`(`||` 解析为 `SqlStdOperatorTable.CONCAT`,即注册的那个 `||` 算符)。
