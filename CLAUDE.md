...
**Calcite 集成(`calcite/`):**
- `calcite/CalciteContext` — parser 配置(MYSQL lex,大小写不敏感,DDL parser factory)。`parse(sql)` → SqlNode;`planInCluster(sql, cluster, currentSchema)` → RelRoot(旧 2 参重载委托 `public`)。catalog reader 搜索路径 `["minidb"]`;`MiniDbRootCalciteSchema` 挂在 `minidb` 下,其 `getTableMap()` 返回当前 schema 的表(支持 unqualified 名),`getSubSchemaMap()` 返回所有 schema(支持 `schema.table` 限定名)+ `information_schema`。**列名大小写不敏感靠 `CalciteCatalogReader` 的连接属性 `caseSensitive=false`**(见坑 48)。
...
- `calcite/MiniDbInformationSchemaCalciteSchema` — 只读系统 schema,`getTableMap()` 暴露 `schemata`/`tables`/`columns` 三表(用固定 `TableSchema`,见 `exec/InformationSchema`)。
...
- `exec/InformationSchema` — 只读 `information_schema` 系统表物化:`SCHEMA_NAME="information_schema"`、`isSystemSchema(name)`、`schemataSchema()/tablesSchema()/columnsSchema()`(固定 `TableSchema`,列名标准大写)、`materialize(catalog, tableName, allocator) → VectorSchemaRoot`(从内存 catalog 物化行,见坑 50)。
...
50. **information_schema 系统表接线**:`InformationSchema`(exec/)从内存 catalog 物化 schemata/tables/columns 成 Arrow 批次,列名用**标准大写**(与 JDBC `MetadataExecutor` 一致);`MiniDbInformationSchemaCalciteSchema`(calcite/)把 3 张表暴露给 Calcite,`MiniDbRootCalciteSchema.getSubSchemaMap()` 追加 `information_schema` 子 schema;`MiniDbScan.execute()` 在 3 段限定名里 `schemaName` 命中 `InformationSchema.isSystemSchema` 时走 `InformationSchema.materialize` 单批分支(复用 `singleBatch` 包装);`QueryExecutor.handleCreateSchema` 拒绝 `information_schema` 保留名。注意:information_schema 表读的是**用户** catalog(不含那 3 张系统表自身);大量 NULL 列(如 `CATALOG_NAME`)靠「只对有值列 `setSafe`、全部列 `setValueCount(n)`」实现(先 setValueCount 再 `VectorSchemaRoot.of`,见坑 26)。
