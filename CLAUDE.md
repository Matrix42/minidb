...
55. **约束(主键/唯一/NOT NULL)落点**:约束元数据存 `TableSchema`(`primaryKey`/`uniqueKeys`,空=无)+ `ColumnMeta.nullable`(默认 true,旧 catalog.json 归一化),随 catalog.json 持久化。`MiniDbCalciteTable.getStatistic()` 把主键+唯一约束转成 `ImmutableBitSet` 作唯一键(**与统计无关,未 ANALYZE 也提供**),供 CBO 的 `RelMdUniqueKeys`→`RelMdDistinctRowCount` 得到更准的 distinct/join 基数。DDL:表级 `PRIMARY KEY (col)`/`UNIQUE (col)` 是 Calcite 原生 `SqlKeyConstraint`(用 `key.getOperandList().get(1)` 取列名,`getColumnList()` 不存在);**列级 `col TYPE PRIMARY KEY`/`UNIQUE` Calcite 不支持**,`QueryExecutor.rewriteColumnConstraints` 在 parse 前用字符串扫描(跟踪括号深度/引号)把列级约束提升为表级。执行期校验在 `MiniDbModify.validateInsert`(INSERT 前):NOT NULL + 主键/唯一冲突,含 null 的键不参与唯一性(唯一约束允许多 null);校验失败走 try-finally 关闭输入迭代器(否则 Arrow 内存泄漏)。**外键留 Phase 2**:Calcite 无 FOREIGN KEY 的 DDL 解析(无 `SqlForeignKeyConstraint` 类),需自定义解析 + 引用完整性校验。
