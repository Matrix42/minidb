...
- `rule/logical/MiniDbLogicalRules` — 逻辑规则集(Calcite 内置,`Config.DEFAULT.toRule()` 构造),分两组:
  - `HEP`(HepPlanner 阶段,不依赖 trait):`FilterJoinRule.FilterIntoJoinRule`(FilterPushDown 进 join)/`FilterProjectTransposeRule`/`ProjectMergeRule`/`FilterMergeRule` + `ProjectToCalcRule`/`FilterToCalcRule`/`CalcMergeRule`(Project+Filter 合并为单个 Calc)+ `SortRemoveConstantKeysRule`/`SortRemoveDuplicateKeysRule`/`SortRemoveRedundantRule`(常量/重复/冗余排序键移除)。
...
- `plan/physical/`(`com.minidb.server.plan.physical`)——MiniDb* 物理算子,均 `implements MiniDbRel`:
  ...
  - `MiniDbFilter` — 过滤,自己分配输出 batch,`owned` 队列跟踪待关闭。**防御性**:常规 SQL 的 Filter 经 FilterToCalc 转成 Calc,此算子仅在无 Calc 转换时兜底。
  - `MiniDbProject` — 投影,RexInterpreter 求值后重命名列。**防御性**:常规 SQL 的 Project 经 ProjectToCalc 转成 Calc;窗口函数(RexOver)仍可能走此算子的 eager 路径(见 WindowFunctions)。
  ...
  - `MiniDbCalc` — Calc(Project+Filter 泛化),`Calc` 子类,**主路径算子**:`ProjectToCalcRule`/`FilterToCalcRule`/`CalcMergeRule` 把常规 SQL 的 Project+Filter 合并为单个 Calc。**lazy 流式**:`program.getProjectList()` 逐个 `expandLocalRef` + `getCondition()`,eval 求值,按条件行过滤后 `renameFiltered` 拷入新向量。**eager 窗口路径**:投影含 `RexOver` 时物化输入、先按 condition 过滤(SQL WHERE 先于窗口)、`RexShuttle.visitOver` 提取窗口列、`WindowFunctions.computeOver` 算值、eval 改写投影(与 MiniDbProject 的窗口路径同构)。
  ...
  - `WindowFunctions` — 窗口函数静态工具:`materialize` + `computeOver(RexOver, rows)`。按 RexWindow 的 partitionKeys 分组、orderKeys 排序,逐行计算:聚合 SUM/AVG/COUNT/MIN/MAX over 帧、ROW_NUMBER/RANK/DENSE_RANK(peers)、LEAD/LAG(offset+default)、FIRST_VALUE/LAST_VALUE。帧=RexWindowBound 转行位置(RANGE/ROWS 同位置语义)。**窗口执行在 MiniDbProject/MiniDbCalc 内嵌**:检测投影含 RexOver → eager 路径,`RexShuttle.visitOver` 提取为窗口列引用后 eval 改写表达式。Calcite 1.42 无 LogicalWindow(ProjectToWindowRule 不可靠),无窗口算子/规则。
...
30. **Calc 现在走主路径(2026-08-13 起)**:注册 `ProjectToCalcRule`/`FilterToCalcRule`/`CalcMergeRule` 后,常规 SQL 的 Project+Filter 合并为单个 LogicalCalc,物理转成 `MiniDbCalc`(主路径算子);`MiniDbFilter`/`MiniDbProject` 退为防御性兜底。Calc 的 RexProgram 手动构造(测试用)注意:`RexProgramBuilder.addProject(RexNode,String)`/`addCondition(RexNode)`,无 register 方法;`RexBuilder` 无 `makeLiteral(int)`,用 `makeExactLiteral(BigDecimal, type)` 保类型;`SqlStdOperatorTable` 在 sql.fun 包。
